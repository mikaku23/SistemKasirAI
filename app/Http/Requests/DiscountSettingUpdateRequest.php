...
class DiscountSettingUpdateRequest extends FormRequest
{
    ...
    {
        $setting = $this->route('discount_setting');
        $settingId = $setting instanceof DiscountSetting ? $setting->id : $setting;

        return [
            'id' => ['nullable', 'integer'],
            'name' => ['required', 'string', 'max:150', Rule::unique('discount_settings', 'name')->ignore($settingId)],
            'code' => ['nullable', 'string', 'max:50', Rule::unique('discount_settings', 'code')->ignore($settingId)],
