...
<section class="page-card glass-card role-page">
    <div class="page-card__head">
        <div class="page-card__title">
            ...
            <h2>Edit Supplier</h2>
            <p>Ubah data supplier bertahap sebelum disimpan kembali.</p>
        </div>
    ...
    <form
        ...
        data-step-form
        data-draft-key="suppliers:edit:{{ $supplier['id'] }}"
        data-confirm-form
        data-confirm-title="Simpan perubahan supplier?"
        data-confirm-message="Data supplier yang diubah akan disimpan ke database. Lanjutkan?"
            ...
            <section class="wizard-step" data-step="3">
                ...
                <div class="review-grid">
                    <div class="review-item">
                        <span>Name</span>
                        <strong data-review-field="name">{{ $supplier['name'] ?? '-' }}</strong>
                    </div>
                    <div class="review-item">
                        <span>Code</span>
                        <strong data-review-field="code">{{ $supplier['code'] ?? '-' }}</strong>
                    </div>
                    <div class="review-item">
                        <span>Phone</span>
                        <strong data-review-field="phone">{{ $supplier['phone'] ?? '-' }}</strong>
                    </div>
                    <div class="review-item">
                        <span>Email</span>
                        <strong data-review-field="email">{{ $supplier['email'] ?? '-' }}</strong>
                    </div>
                    <div class="review-item review-item--full">
                        <span>Address</span>
                        <p data-review-field="address">{{ $supplier['address'] ?? '-' }}</p>
                    </div>
                    <div class="review-item review-item--full">
                        <span>Notes</span>
                        <p data-review-field="notes">{{ $supplier['notes'] ?? '-' }}</p>
                    </div>
                    <div class="review-item">
                        <span>Status</span>
                        <strong data-review-field="status" data-review-source="is_active">{{ ($supplier['is_active'] ?? 1) ? 'Active' : 'Inactive' }}</strong>
                    </div>
                </div>
            </section>
