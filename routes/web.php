...
use App\Http\Controllers\AiChannelController;
...
Route::middleware(['auth', 'role:Admin'])->group(function () {
    ...
    Route::get('ai-core', [AiCoreController::class, 'index'])->name('ai-core.index');
    Route::post('ai-core/dispatch', [AiCoreController::class, 'dispatch'])->name('ai-core.dispatch');
    Route::post('ai-core/guard', [AiCoreController::class, 'guard'])->name('ai-core.guard');
    Route::get('ai-channels', [AiChannelController::class, 'index'])->name('ai-channels.index');
    ...
    Route::get('stock-batches/recycle', [StockBatchController::class, 'recycle'])->name('stock-batches.recycle');
    Route::post('stock-batches/{stock_batch}/restore', [StockBatchController::class, 'restore'])->whereNumber('stock_batch')->name('stock-batches.restore');
    Route::delete('stock-batches/{stock_batch}/force-delete', [StockBatchController::class, 'forceDelete'])->whereNumber('stock_batch')->name('stock-batches.forceDelete');
    Route::resource('stock-batches', StockBatchController::class)->parameters(['stock-batches' => 'stock_batch']);

    Route::get('supplier-returns/recycle', [SupplierReturnController::class, 'recycle'])->name('supplier-returns.recycle');
    Route::post('supplier-returns/{supplier_return}/restore', [SupplierReturnController::class, 'restore'])->whereNumber('supplier_return')->name('supplier-returns.restore');
    Route::delete('supplier-returns/{supplier_return}/force-delete', [SupplierReturnController::class, 'forceDelete'])->whereNumber('supplier_return')->name('supplier-returns.forceDelete');
    Route::resource('supplier-returns', SupplierReturnController::class)->parameters(['supplier-returns' => 'supplier_return']);
    ...
    Route::get('log-tc/{transaction}', [LogTcController::class, 'show'])->whereNumber('transaction')->name('log-tc.show');

    Route::get('locations/recycle', [LocationController::class, 'recycle'])->name('locations.recycle');
    Route::post('locations/{location}/restore', [LocationController::class, 'restore'])->whereNumber('location')->name('locations.restore');
    Route::delete('locations/{location}/force-delete', [LocationController::class, 'forceDelete'])->whereNumber('location')->name('locations.forceDelete');
    Route::resource('locations', LocationController::class);

    Route::get('tax-settings/recycle', [TaxSettingController::class, 'recycle'])->name('tax-settings.recycle');
    Route::post('tax-settings/{tax_setting}/restore', [TaxSettingController::class, 'restore'])->whereNumber('tax_setting')->name('tax-settings.restore');
    Route::delete('tax-settings/{tax_setting}/force-delete', [TaxSettingController::class, 'forceDelete'])->whereNumber('tax_setting')->name('tax-settings.forceDelete');
    ...
    Route::get('stock-movements/{stockMovement}', [StockMovementController::class, 'show'])->whereNumber('stockMovement')->name('stock-movements.show');
    ...
    Route::get('/visitors/{visitor}', [VisitorController::class, 'show'])->whereNumber('visitor')->name('visitors.show');
});

Route::middleware(['auth', 'role:Admin,Manager'])->group(function () {
    Route::post('ai/chatbot', [AiCoreController::class, 'chatbot'])->name('ai-core.chatbot');
});

Route::middleware(['auth', 'role:Admin,Gudang,Warehouse,Storehouse'])->group(function () {
    Route::post('ai/search', [AiCoreController::class, 'search'])->name('ai-core.search');
});

Route::middleware(['auth', 'role:Admin,Cashier,Kasir,Gudang,Warehouse,Storehouse'])->group(function () {
    Route::post('ai/customer-service', [AiCoreController::class, 'customerService'])->name('ai-core.customer-service');
});
